...
- [ ] Schema v1 frozen as `schema/gesture-macro-v1.json` in the repo (extracted from ARCHITECTURE.md, which then references the file).
- [ ] `@Serializable` Kotlin models in `core/serialization` mirroring the schema (sealed `Action` hierarchy by `type`; sealed/validated trigger patterns).
- [ ] **Strict import:** unknown fields rejected, schema violations produce field-level error messages, no partially-applied imports (all-or-nothing per document).
- [ ] `version` dispatch: importer reads `version` first and routes to the right decoder; importing an unsupported version fails with a clear message.
